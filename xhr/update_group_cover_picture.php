...
if ($f == 'update_group_cover_picture') {
    if (isset($_FILES['cover']['name']) && !empty($_POST['group_id']) && is_numeric($_POST['group_id']) && $_POST['group_id'] > 0) {
        // SECURITY: Verify group ownership before allowing cover upload
        $group_check = Wo_GroupData($_POST['group_id']);
        if (empty($group_check) || ($group_check['user_id'] != $wo['user']['user_id'] && !Wo_IsAdmin())) {
