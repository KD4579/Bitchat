...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
                ...
                if (!empty($matches1) && !empty($matches1[2])) {
                    foreach ($matches1[2] as $key => $title) {
                        if (strpos(strtolower($title), strtolower($keyword)) !== false) {
                            ...
                            $html .= '<a href="' . Wo_LoadAdminLinkSettings($file) . '?highlight=' . $keyword . '" "><div style="font-weight:600;font-size:13px;">' . $page_title . '</div><div><small>' . $title . '</small></div></a>';
                            break;
                        }
                    }
                }
                ...
                if (!empty($matches2) && !empty($matches2[2])) {
                    foreach ($matches2[2] as $key => $lable) {
                        if (strpos(strtolower($lable), strtolower($keyword)) !== false) {
                            ...
                            $html .= '<a href="' . Wo_LoadAdminLinkSettings($file) . '?highlight=' . $keyword . '" "><div style="font-weight:600;font-size:13px;">' . $page_title . '</div><div><small>' . $lable . '</small></div></a>';
                            break;
                        }
                    }
                }
    ...
    if ($s == 'approve_receipt') {
        if (!empty($_GET['receipt_id'])) {
            ...
            $receipt = $db->where('id', $id)->getOne('trdc_payments', array(
                '*'
            ));
            if ($receipt) {
                $updated = $db->where('id', $id)->update('trdc_payments', array(
                    'approved' => 1,
                    'approved_at' => time()
                ));
                $updated = true;
    ...
    if ($s == 'delete_receipt') {
        if (!empty($_GET['receipt_id'])) {
            ...
            Wo_RegisterNotification($notification_data_array);
            $db->where('id', $id)->delete('trdc_payments');
            if (file_exists($photo_file)) {
                @unlink(trim($photo_file));
            } else if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                @Wo_DeleteFromToS3($photo_file);
